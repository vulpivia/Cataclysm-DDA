...
def parse_furnter(om_objs, palette, conflicts):
    for map_key, map_val in om_objs.items():
        ...
        if not pal_val:
            ...
            conflicts[map_key].setdefault(
                map_hash, {"val": map_val, "count": 0})
            conflicts[map_key].setdefault(
                pal_hash, {"val": pal_val, "count": 0})
...
args.add_argument("palette_name", action="store",
                  help="specify the name of the resulting palette.")
args.add_argument("palette_path", action="store",
                  help="specify the folder for the palette.")
...
# first pass, go through and find all the existing terrains and furnitures and
# any conflicts
