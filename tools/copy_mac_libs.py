...
def copy_and_rewrite(file):
    ...
    if otool_ret.returncode != 0:
        raise Exception(
            f"An error occured in calling otool -L:\n {otool_ret.stderr.decode('utf-8')}"
        )
    dependencies = []
    for line in otool_ret.stdout.decode('utf-8').split('\n')[1:]:
        ...
        if len(line) == 0 or line.find(" (compatibility version") == -1:
            continue
        library = os.path.abspath(line[:line.find(" (compatibility version")])
