...
OBSOLETE_TOPICS = [
    "TALK_DENY_GUARD", "TALK_FRIEND_UNCOMFORTABLE", "TALK_USE_ITEM",
    "TALK_GIVE_ITEM"
]
...
args.add_argument(
    "dialogue_json", nargs="+", action="store",
    help="specify json folder to validate.  The valdiator will walk the "
         "folder's tree and validate all JSON files in it.  Use "
         "'data/json/npcs/ to validate the dialogue in the vanilla game.")
...
def add_topic_by_chat(topics, topic_id, topic_branches=None, this_ids=None):
    ...
    if topic_branches is not None:
        topic_branches.setdefault(
            topic_id, {"responses": [], "ends": False, "parent": None})
...
def add_topic_by_response(topics, response, topic_branches=None,
                          this_ids=None):
...
def validate(dialogue):
    ...
    topic_branches = {
        "TALK_TRAIN": {"responses": ["TALK_TRAIN_START"], "ends": False,
                       "parent": None},
        "TALK_HOW_MUCH_FURTHER": {"responses": ["TALK_DONE"], "ends": True,
                                  "parent": None},
        "TALK_SEDATED": {"responses": ["TALK_DONE"], "ends": True,
                         "parent": None},
    }
    # defined in src/npctalk.cpp
    defined_ids = [
        "TALK_NONE", "TALK_DONE", "TALK_TRAIN", "TALK_HOW_MUCH_FURTHER",
        "TALK_SEDATED"
    ]
    ...
    for talk_topic in dialogue:
        ...
        if isinstance(topic_id, list):
            for loop_topic_id in topic_id:
                add_topic_by_id(topics, loop_topic_id, topic_branches,
                                this_ids)
        else:
        ...
        for topic_id, branch_record in topic_branches.items():
            ...
            for response_id in branch_record["responses"]:
                if topic_branches.get(response_id, {}).get("ends", False):
                    ...
                    #print("pass {} {} terminates by {}".format(
                    #    passes, topic_id, response_id))
    ...
    skip_topics = ["TALK_DONE", "TALK_SIZE_UP", "TALK_LOOK_AT", "TALK_OPINION",
                   "TALK_SHOUT"]
